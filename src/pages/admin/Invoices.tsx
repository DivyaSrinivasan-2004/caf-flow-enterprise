...
const Invoices = () => {
  ...
  const mapStatus = (paymentRaw: unknown, orderRaw: unknown, dueDate: string): InvoiceStatus => {
    ...
    if (payment.includes("paid")) return "paid";

    // If payment status is absent/unknown, keep invoice unpaid by default.
    if (payment.includes("pending") || payment.includes("unpaid") || !payment) {
    ...
    const loadInvoices = async () => {
      try {
        ...
        const mapped: InvoiceRow[] = list.map((row: Record<string, unknown>) => {
          ...
          return {
            id: row.bill_number ? `INV-${String(row.bill_number)}` : `INV-${orderPk.slice(0, 8).toUpperCase()}`,
  ...
  const handleDownloadInvoicePdf = () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(14);
    doc.text("Invoice Detail", 14, 14);
    doc.setFontSize(10);
    doc.text(`Bill No: ${selectedInvoice.bill_number}`, 14, 20);
    doc.text(`Customer: ${selectedInvoice.customer_name}`, 14, 26);
    doc.text(`Date: ${selectedInvoice.date}`, 14, 32);
    doc.text(`Payment: ${selectedInvoice.payment_method}`, 14, 38);
    doc.text(`Status: ${selectedInvoice.payment_status}`, 14, 44);

    autoTable(doc, {
      startY: 50,
      head: [["Item", "Qty", "Price", "GST %", "GST", "Total"]],
      body: selectedInvoice.line_items.map((li) => [
        li.name,
        String(li.quantity),
        li.base_price.toFixed(2),
        li.gst_percent.toFixed(2),
        li.gst_amount.toFixed(2),
    ...
    autoTable(doc, {
      startY: (doc as jsPDF & { lastAutoTable?: { finalY?: number } }).lastAutoTable?.finalY
        ? ((doc as jsPDF & { lastAutoTable?: { finalY?: number } }).lastAutoTable?.finalY ?? 50) + 8
        : 50,
      body: [
        ["Subtotal", `Rs.${selectedInvoice.subtotal.toLocaleString()}`],
        ["Total GST", `Rs.${selectedInvoice.total_gst.toLocaleString()}`],
        ["Discount", `Rs.${selectedInvoice.discount.toLocaleString()}`],
        ["Final Amount", `Rs.${selectedInvoice.final_amount.toLocaleString()}`],
      ],
      theme: "plain",
      styles: { fontSize: 10 },
    ...
    doc.save(`invoice-${selectedInvoice.bill_number || "detail"}.pdf`);
  };
            ...
            <div className="p-4 bg-white text-slate-900">
              <div className="mx-auto w-full max-w-md rounded-md border border-dashed border-slate-300 p-4 font-mono text-xs">
                <div className="text-center border-b border-dashed border-slate-300 pb-3">
                  ...
                  <p className="mt-1">Bill No: {selectedInvoice.bill_number}</p>
                ...
                <div className="py-3 space-y-1 border-b border-dashed border-slate-300">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{selectedInvoice.order_type}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Staff</span>
                  ...
                  <div className="flex justify-between">
                    <span>Status</span>
                    <span>{selectedInvoice.payment_status}</span>
                  </div>
                </div>

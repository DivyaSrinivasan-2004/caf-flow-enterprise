...
import { AlertCircle, CalendarCheck2, Check, ChevronDown, Clock3, Search, ShieldCheck, UserCog, Users2, X } from "lucide-react";
...
const StaffManagement = () => {
  ...
  const loadAttendanceLogs = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/reports/staff/login-logout/`, { headers: getAuthHeaders() });
      ...
      const list = Array.isArray(data)
        ...
        : Array.isArray((data as { results?: unknown[] }).results)
          ...
          : Array.isArray((data as { data?: unknown[] }).data)
            ...
            : [];
      setAttendanceLogs(list as AttendanceLog[]);
    } catch (err) {
      console.error(err);
      setAttendanceLogs([]);
    }
  ...
  const attendance = useMemo(() => {
    ...
    const recent = Object.values(latestByStaff)
      .sort((a, b) => new Date(b.login || 0).getTime() - new Date(a.login || 0).getTime())
      .slice(0, 5);

    const present = recent.length;
    const late = recent.filter((entry) => {
